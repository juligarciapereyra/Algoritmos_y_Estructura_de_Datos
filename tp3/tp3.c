...
unsigned long int index_key(const char *key, int capacity, dictionary_t *dictionary){
  if(dictionary -> size != 0){
    unsigned long int index = index_hash(key, capacity); 

    for(int i = 0; i < capacity; i++){
      unsigned long int position = (index + i) % capacity;
  ...
  for(int i = 0; i < dictionary -> capacity; i++){
    ...
    if((dictionary -> table[position].key == NULL)&& (dictionary -> table[position].is_deleted == false)){
      ...
      if(pos == -1){
        ...
        dictionary -> table[pos].is_deleted = false;
        dictionary -> size++; 
        return true;
      }
    ...
    }else if(strcmp(dictionary->table[position].key, key) == 0){
